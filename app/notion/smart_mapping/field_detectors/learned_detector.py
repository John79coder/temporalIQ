...
class LearnedDetector(FieldDetector):
    def __init__(self, features_service: FeaturesService, ai_data_service: AIDataService, logging_service: LoggingService):
    ...
    def _fetch_feedback(self, db: Session, user_id: int, scope: str) -> List:
        ...
        try:
            ...
            return repo.get_all_feedback(db) if scope == 'global' else db.query(FeedbackLog).filter_by(user_id=user_id).all()
    ...
    def _train_model(self, db: Session, user_id: int) -> bool:
        ...
        try:
            ...
            if len(feedbacks) < 50:
                self.logging_service.info("Insufficient feedback for training, using fallback", user_id=user_id, extra={"feedback_count": len(feedbacks)})
                return False
            fields, concepts = zip(*[(f.notion_field + (" " + f.feedback_text if f.feedback_text else ""), f.corrected_concept) for f in feedbacks])
            ...
            if acc < 0.85:
                self.logging_service.info("Model accuracy too low on holdout, using fallback", user_id=user_id, extra={"accuracy": acc})
                return False
            scores = cross_val_score(self.model, X_train_vec, y_train, cv=5)
            if scores.mean() < 0.85:
                self.logging_service.info("Cross-validation score too low, using fallback", user_id=user_id, extra={"cv_mean": scores.mean()})
                return False
            try:
                self._save_model()
                self.trained = True
    ...
    def _log_mapping_event(self, db: Session, user_id: int, field_name: str, concept: str):
        ...
        try:
            ...
            self.logging_service.error("Failed to log mapping event", user_id=user_id, extra={"error": str(e), "field_name": field_name})
    ...
    def detect(self, fields: list[dict], rows: List[dict] = None, db: Session = None, user_id: int = None) -> list[FieldMatch]:
        ...
        try:
            ...
            try:
                ...
                self.logging_service.error("Transaction failed in detection logging", user_id=user_id, extra={"error": str(e)})
                raise wrap_external_error(e, DatabaseError, "Failed to log mapping events") from e
            return matches
        except ValueError as e:
            self.logging_service.error("Invalid input for detection", user_id=user_id, extra={"error": str(e), "fields": field_names})
            return []
        except SQLAlchemyError as e:
