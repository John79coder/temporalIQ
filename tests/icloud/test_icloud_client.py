...
def test_caldav_client_list_calendars(caldav_client):
    ...
    caldav_client.list_calendars.return_value = [CalendarMetadata(calendar_id="cal1", display_name="Calendar 1", timezone="UTC")]
...
def test_caldav_client_fetch_events(caldav_client):
    ...
    caldav_client.fetch_events.return_value = [CalendarEvent(start=datetime.now(), end=datetime.now(), summary="Event", uid="uid1")]
