...
class DescriptionExtractor(PageValueExtractor):
    ...
    def extract(self, section_blocks: List[Dict], db: Session, user_id: int) -> PartialCandidate:
        ...
        return PartialCandidate(description=description,
                                confidence=confidence)  # Description not in schema; add field if needed (e.g., description: Optional[str] = None in Partial/TaskCandidate)
