...
T5_CHECKPOINT = "unikei/t5-base-split-and-rephrase"
model_dir = current_app.config.get("MODEL_DIR", ".")
checkpoint_path = os.path.join(model_dir, T5_CHECKPOINT)
...
def split_with_spacy(sentence: str) -> ProcessorResult:
    doc = nlp(sentence)
...
def split_with_t5(sentence: str, max_output_segments=5) -> ProcessorResult:
    input_text = f"split: {sentence}"
    inputs = tokenizer(input_text, return_tensors="pt", truncation=True, max_length=256)
    outputs = t5_model.generate(inputs.input_ids, num_beams=4, max_length=256, early_stopping=True)
...
def ensemble_split(sentence: str) -> List[str]:
    # Run both processors
    spa = split_with_spacy(sentence)
    t5 = split_with_t5(sentence)
...
def split_into_tasks(sentence: str) -> List[str]:
    segments = ensemble_split(sentence)
