...
def test_preview_schedule_success(mock_fetch_user_events, authorized_client, db_session, app, test_user):
    ...
    with app.app_context():
        ...
        db_session.add(
            TaskCandidate(user_id=user_id, notion_db_id="db1", title="Test Task", duration=30, confidence=1.0,
                          due_date=None, issues=[]))
        ...
        for i in range(100):
            ...
            duration = max(15, min(120,
                                   20 + (30 * urgency) - (2 * num_events) + (5 * day_length_hours) + random.uniform(-10,
                                                                                                                    10)))
...
def test_confirm_schedule_success(mock_get_caldav_client, mock_write_scheduled_event, authorized_client, db_session,
                                  app, test_user):
    _, user_id = test_user
