...
def time_block_generator(caching_service, task_prioritizer, features_service, ai_data_service, logging_service):
    return TimeBlockGenerator(caching_service, None, task_prioritizer, features_service, ai_data_service, logging_service)

@pytest.fixture
def free_time_finder(caching_service, event_service, features_service, preferences_service, ai_data_service, logging_service, time_block_generator):
    ftf = FreeTimeFinder(caching_service, event_service, features_service, preferences_service, ai_data_service, logging_service, time_block_generator)
...
def test_time_block_generator_generate(mock_prioritize_tasks, mock_find_free_slots, mock_get_candidates, time_block_generator, free_time_finder, task_prioritizer):
    mock_database_session = Mock()

    mock_find_free_slots.return_value = [
        ICloudTimeBlock(start=datetime(2025, 7, 12, 9, 0, tzinfo=timezone.utc), end=datetime(2025, 7, 12, 9, 30, tzinfo=timezone.utc))
    ]
    ...
    with patch('app.features.services.service.FeaturesService.get_settings') as mock_get_features_settings:
        ...
        with patch.object(time_block_generator, '_log_urgency_event', return_value=None) as mock_log_urgency:
            blocks = time_block_generator.generate_time_blocks(1, mock_database_session, "db1", "cal1", datetime(2025, 7, 12), datetime(2025, 7, 12), "09:00", "17:00")
...
def test_free_time_finder_find_slots(free_time_finder, caching_service, event_service):
    ...
    with patch.object(free_time_finder.features_service, 'get_settings') as mock_get_settings, \
        ...
        mock_get_preferences.return_value = UserPreferences(user_id=1, time_zone="UTC", work_hours=8.0, block_size_minutes=30)
