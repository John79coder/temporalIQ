...
def test_invalid_token_access_denied(client):
    response = client.post("/notion/connect", json={"user_id":1,"code":"code","redirect_uri":"http://localhost"}, headers={"X-CSRF-Token": client.csrf_token, "Authorization": "Bearer invalid"})
    assert response.status_code == 401


def test_xss_prevention_in_routes(authorized_client, app, test_user):
    payload = {"user_id": test_user[1], "block_size_minutes": "<script>alert(1)</script>", "allow_weekends": False, "max_blocks_per_day": 16, "work_hours": 7.6}
...
def test_sql_injection_vulnerability(authorized_client, app):
    ...
    response = authorized_client.post("/auth/login", json=payload, headers={"X-CSRF-Token": authorized_client.csrf_token})
    assert response.status_code == 401