...
class LearnedDetector(FieldDetector):
    ...
    def _fetch_feedback(self, db: Session, user_id: int, scope: str) -> List[FeedbackLog]:
        """Fetch feedback data for training based on scope."""
        try:
            repo = FeedbackRepository()
            if scope == "global":
                return repo.get_all_feedback(db)
            return db.query(FeedbackLog).filter_by(user_id=user_id).all()
        ...
        try:
            ...
            if len(feedbacks) < 50:
                self.logging_service.info(
                    "Insufficient feedback for training, using fallback",
                    user_id=user_id,
            ...
            fields, concepts = zip(
                *[
                    (
                        f.notion_field
                        + (" " + f.feedback_text if f.feedback_text else ""),
                        f.corrected_concept,
                    )
                    for f in feedbacks
                ]
            ...
            if acc < 0.85:
                self.logging_service.info(
                    "Model accuracy too low on holdout, using fallback",
                    user_id=user_id,
                    extra={"accuracy": acc},
                )
                return False
            ...
            if scores.mean() < 0.85:
                self.logging_service.info(
                    "Cross-validation score too low, using fallback",
                    user_id=user_id,
            ...
            self.logging_service.error(
                "Database error during training",
                user_id=user_id,
                extra={"error": str(e)},
            )
            raise wrap_external_error(e, DatabaseError, "Failed to train learned model") from e

    def _log_mapping_event(self, db: Session, user_id: int, field_name: str, concept: str) -> None:
    ...
    def detect(
        ...
        try:
            ...
            if not self.trained:
                try:
                    model_dir = current_app.config.get("MODEL_DIR", ".")
                    self.vectorizer, self.model = joblib.load(
                        os.path.join(model_dir, "learned_model.pkl")
                    )
                    self.trained = True
            ...
            try:
                ...
                self.logging_service.error(
                    "Transaction failed in detection logging",
                    user_id=user_id,
                    extra={"error": str(e)},
                )
            ...
            self.logging_service.error(
                "Invalid input for detection",
                user_id=user_id,
                extra={"error": str(e), "fields": [f["name"] for f in fields]},
            )
            return []

        except SQLAlchemyError as e:
            self.logging_service.error(
                "Database error in detection",
                user_id=user_id,
                extra={"error": str(e)},
            )
